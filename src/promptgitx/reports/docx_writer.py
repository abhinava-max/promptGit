...
def add_heading(document: Document, text: str, level: int = 1):
    ...
    for run in heading.runs:
        ...
        run.font.color.rgb = RGBColor.from_string(COLOR_PURPLE if level == 1 else COLOR_INDIGO)
...
def add_key_value_table(document: Document, rows: list[tuple[str, Any, str | None]]):
    ...
    for label, value, color in rows:
        ...
        set_cell_shading(row.cells[0], "EEF2FF")
...
def add_files_table(document: Document, files: list[dict[str, Any]]):
    ...
    for index, header in enumerate(headers):
        ...
        style_paragraph(cell.paragraphs[0], bold=True, color=COLOR_INDIGO)
    ...
    for file in files:
        ...
        style_paragraph(row.cells[0].paragraphs[0], color=COLOR_GREEN, bold=True)
...
def add_issue_section(document: Document, title: str, items: list[dict[str, Any]]):
    ...
    for item in items:
        ...
        add_colored_run(paragraph, str(item.get("file_path", "unknown")), COLOR_GREEN, bold=True)
        add_colored_run(paragraph, ": ", COLOR_DARK)
        add_colored_run(
            paragraph,
...
def add_file_wise_issues(document: Document, files: list[dict[str, Any]]):
    ...
    for file in files:
        for issue in file.get("issues", []):
            ...
            row = table.add_row()
            cell = row.cells[0]
            set_cell_border(cell, severity_color(issue.get("severity")))
            ...
            add_colored_run(paragraph, title_case(issue.get("category")), COLOR_INDIGO, bold=True)
            ...
            for label, value in details:
                ...
                color = COLOR_GREEN if label == "File" else COLOR_DARK
...
def add_clean_checks(document: Document, issues: dict[str, list[dict[str, Any]]]):
    ...
    if clean:
        ...
        add_colored_run(paragraph, ", ".join(clean), COLOR_GREEN, bold=True)
    else:
...
def create_docx_report(report: dict[str, Any], output_path: Path) -> None:
    ...
    add_colored_run(title, "GitPromptX Review Report", COLOR_PURPLE, bold=True, size=24)
